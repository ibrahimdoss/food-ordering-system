...
public class PaymentDomainServiceImpl implements PaymentDomainService{
    ...
    private void validateCreditEntry(Payment payment, CreditEntry creditEntry, List<String> failureMessages) {
        if (payment.getPrice().isGreaterThan(creditEntry.getTotalCreditAmount())) {
            ...
            failureMessages.add("Customer with id=" + payment.getCustomerId().getValue() + " doesn't have enough credir for payment!");
        }
    }
    ...
    //Asagidaki metotta müşterinin ödemeyi tamamlayabilmesi için yeterli kredisi olup olmadığını kontrol ediyoruz. ve kredi borçlarından büyük olmalıdır.
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        ...
        if (!creditEntry.getTotalCreditAmount().equals(totalCreditHistory.subtract(totalDebitHistory))){
            log.error("Credit history total is not equal to current credit for customer id: {}!", creditEntry.getId().getValue());
            failureMessages.add("Credit history total is not equal to current credit for customer id: {}!" + creditEntry.getId().getValue() + "!");
        }
