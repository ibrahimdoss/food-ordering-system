...
public class PaymentResponseKafkaListener implements KafkaConsumer<Envelope> {
    ...
    public PaymentResponseKafkaListener(PaymentResponseMessageListener paymentResponseMessageListener, OrderMessagingDataMapper orderMessagingDataMapper, KafkaMessageHelper kafkaMessageHelper) {
    ...
    public void receive(@Payload List<Envelope> messages,
        ...
        messages.forEach(avroModel -> {
            if (avroModel.getBefore() == null && DebeziumOp.CREATE.getValue().equals(avroModel.getOp())) {
                ...
                try {
                    if (PaymentStatus.COMPLETED.name() == paymentOrderEventPayload.getPaymentStatus()) {
                        log.info("Processing successful payment response for order id:{}", paymentOrderEventPayload.getOrderId());
                    ...
                    } else if (PaymentStatus.CANCELLED.name() == paymentOrderEventPayload.getPaymentStatus() ||
                            PaymentStatus.FAILED.name() == paymentOrderEventPayload.getPaymentStatus()) {
                        log.info("Processing failed payment response for order id:{}", paymentOrderEventPayload.getOrderId());
                    ...
                    log.error("Order not found for order id: {}", paymentOrderEventPayload.getOrderId());
                    ...
                    if (sqlException != null && sqlException.getSQLState() != null &&
                        ...
                        log.error("Caught unique constraint exception with sql state: {} " +
                                "in PaymentRequestKafkaListener for order id: {}", sqlException.getSQLState(), paymentOrderEventPayload.getOrderId());
                        {
